...
func TestRecordAPIRequestWritesOnlyClaudeOAuthOutboundLog(t *testing.T) {
	logRoot := t.TempDir()
...
func TestRecordAPIRequestSkipsOtherOutboundLogs(t *testing.T) {
	logRoot := t.TempDir()
	t.Setenv("WRITABLE_PATH", logRoot)

	enabled := &config.Config{SDKConfig: config.SDKConfig{RequestLog: true}}
	RecordAPIRequest(context.Background(), enabled, UpstreamRequestLog{
	...
	RecordAPIRequest(context.Background(), &config.Config{}, UpstreamRequestLog{
		URL:       "https://api.anthropic.com/v1/messages",
		Method:    http.MethodPost,
		Body:      []byte(`{}`),
		Provider:  "claude",
		AuthID:    "disabled-claude-oauth",
		AuthType:  "oauth",
		AuthValue: "disabled@example.com",
	})
